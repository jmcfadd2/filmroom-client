...
function Course() {
  ...
  const purchaseIds = useSelector(state => state.user.credentials.coursePurchases)
  useEffect(() => {
  ...
  const isPurchased = () => {
    if (purchaseIds.includes(courseId))
      return true
    else return false
  }
  return (
    <div>
      ...
      <Paper className={classes.root}>
        ...
        <Typography variant="body1" color='textSecondary'>{courseInfo.body}</Typography>
      </Paper>
    </div>
  );
}
